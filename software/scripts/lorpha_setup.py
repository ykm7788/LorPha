...
def main():
    ...
    for _, row in rawdata.iterrows():
        ...
        rawdata_name1 = row['Contig'].rsplit('/', 1)[-1]
        rawdata_name2 = rawdata_name1.rsplit('.', 1)[0]

        
        job1_phabox += f"mkdir -p {args.o}/analysis_contig/{row['SampleID']}/phabox ; \n"
        job1_genomad += f"mkdir -p {args.o}/analysis_contig/{row['SampleID']}/genomad ; \n"
        job1_vhost += f"mkdir -p {args.o}/analysis_contig/{row['SampleID']}/vhost ; \n"

        
        job2_phabox += f"pixi run --environment phabox phabox2 --task end_to_end --dbdir {args.d}/phabox --outpth {args.o}/analysis_contig/{row['SampleID']}/phabox --contigs {row['Contig']} --threads {args.t} {parameter.iloc[0,1]} ; \n"
        job2_genomad += f"pixi run --environment genomad genomad end-to-end --cleanup {row['Contig']} --threads {args.t} {parameter.iloc[1,1]} {args.o}/analysis_contig/{row['SampleID']}/genomad {args.d}/genomad ; \n"
        job2_lexicmap1 += f"pixi run lexicmap search -d {args.d}/vhost {row['Contig']} -o {args.o}/analysis_contig/{row['SampleID']}/vhost/contig.vhost.tsv {parameter.iloc[2,1]} --threads {args.t} ; \n"
        job2_lexicmap2 += f"pixi run find_identical {args.o}/analysis_contig/{row['SampleID']}/vhost/contig.vhost.tsv {args.d}/vhost/virus.seqID.txt -format 1 -site1 4 -site2 1 -type 3 > {args.o}/analysis_contig/{row['SampleID']}/vhost/contig.vhost.result.txt ;\n"
        job2_merge += f"pixi run merge.phabox_genomad_vhost_contamination -p {args.o}/analysis_contig/{row['SampleID']}/phabox/final_prediction/final_prediction_summary.tsv -g {args.o}/analysis_contig/{row['SampleID']}/genomad/{rawdata_name2}_summary/{rawdata_name2}_virus_summary.tsv -v {args.o}/analysis_contig/{row['SampleID']}/vhost/contig.vhost.result.txt -o {args.o}/analysis_contig/{row['SampleID']} ; \n"
        job2_seq += f"pixi run seqtk subseq {args.o}/analysis_contig/{row['SampleID']}/phabox/filtered_contigs.fa {args.o}/analysis_contig/{row['SampleID']}/virus.contig.txt > {args.o}/analysis_contig/{row['SampleID']}/virus.pre_check.fa ; \n"


        job3_cluster += f"pixi run mmseqs easy-cluster {row['Contig']} {args.o}/analysis_contig/mmseq/cluster/{row['SampleID']}.cluster {args.o}/analysis_contig/mmseq/tmp {parameter.iloc[6,1]} --threads {args.t} ; \n"
        job3_createdb += f"pixi run mmseqs createdb {args.o}/analysis_contig/mmseq/cluster/{row['SampleID']}.cluster_rep_seq.fasta {args.o}/analysis_contig/mmseq/cluster/{row['SampleID']}.cluster_rep_seq.db ; \n"
        job3_taxonomy += f"pixi run mmseqs taxonomy {args.o}/analysis_contig/mmseq/cluster/{row['SampleID']}.cluster_rep_seq.db {args.d}/gtdb/GTDB {args.o}/analysis_contig/mmseq/tax/{row['SampleID']}.result {args.o}/analysis_contig/mmseq/tmp {parameter.iloc[7,1]} --threads {args.t} ; \n"
        job3_createtsv += f"pixi run mmseqs createtsv {args.o}/analysis_contig/mmseq/cluster/{row['SampleID']}.cluster_rep_seq.db {args.o}/analysis_contig/mmseq/tax/{row['SampleID']}.result {args.o}/analysis_contig/mmseq/tax_out/{row['SampleID']}.tax.txt --threads {args.t} ; \n"
        job3_rep += f"cut -f 1 {args.o}/analysis_contig/mmseq/cluster/{row['SampleID']}.cluster_cluster.tsv | sort | uniq > {args.o}/analysis_contig/mmseq/rep/{row['SampleID']}.cluster.txt ; \n"
        job3_taxid += f"pixi run find_identical {args.o}/analysis_contig/mmseq/rep/{row['SampleID']}.cluster.txt {args.o}/analysis_contig/mmseq/tax_out/{row['SampleID']}.tax.txt -format 1 -site1 1 -site2 1 -type 3 > {args.o}/analysis_contig/mmseq/tax_rep/{row['SampleID']}.rep.tax.txt ; \n"
        job3_repfa += f"pixi run seqtk subseq {row['Contig']} {args.o}/analysis_contig/mmseq/rep/{row['SampleID']}.cluster.txt > {args.o}/analysis_contig/mmseq/rep/{row['SampleID']}.cluster.fa ; \n"

  
        job4_covermContig += f"pixi run coverm contig --coupled {row['Reads_R1']} {row['Reads_R2']} --reference {row['Contig']} {parameter.iloc[8,1]} -o {args.o}/analysis_contig/coverm/{row['SampleID']}.coverm.txt --no-zeros -t {args.t} ; \n"
        job4_ContigID += f"pixi run merge.tax_coverm -t {args.o}/analysis_contig/mmseq/tax_rep/{row['SampleID']}.rep.tax.txt -c {args.o}/analysis_contig/coverm/{row['SampleID']}.coverm.txt -o {args.o}/analysis_contig/mmseq/tax_coverm/{row['SampleID']}.tax.coverm.txt ; \n"
        job4_covermVotu += f"pixi run coverm contig --coupled {row['Reads_R1']} {row['Reads_R2']} --reference {args.o}/analysis_votu/votu.fa {parameter.iloc[8,1]} -o {args.o}/analysis_votu/coverm/{row['SampleID']}.votu.coverm.txt --no-zeros -t {args.t} ; \n"

 
        job5_defensefinder1 += f"pixi run --environment defensefinder defense-finder run {args.o}/analysis_contig/mmseq/rep/{row['SampleID']}.cluster.fa -o {args.o}/analysis_contig/defensefinder --models-dir {args.d}/defensefinder/ -w {args.t} ; \n"
...
"""
    ...
    job2_votu = "\n#### extract vOTUs ####\n"
    job2 = job2_votu + job2_phabox + "\n" + job2_genomad + "\n" + job2_lexicmap1 + "\n" + job2_lexicmap2 + "\n" + job2_merge + "\n" + job2_seq + "\n" + job2_combine + "\n" + job2_checkv + "\n" + job2_checkv_combine + "\n" + job2_filter1 + "\n" + job2_filter2 + "\n" + job2_filter3 + "\n" + job2_vclust1 + "\n" + job2_vclust2 + "\n" + job2_vclust3 + "\n" + job2_votu1 + "\n" + job2_votu2 + "\n" + job2_votu3 + "\n" + job2_votu4 + "\n\n"

    job3_mmseq = "\n#### extract prokaryotes ####\n"
    job3 = job3_mmseq + job3_cluster + "\n" + job3_createdb + "\n" + job3_taxonomy + "\n" + job3_createtsv + "\n" + job3_rep + "\n" + job3_taxid + "\n" + job3_repfa + "\n\n"
    ...
    job4 = job4_coverm + job4_covermContig + "\n" + job4_ContigID + "\n" + job4_bacteria_merger + "\n" + job4_covermVotu + "\n" + job4_votu_merger + "\n\n"
    ...
    job5_votutaxa = f"pixi run merge_votu_taxa -v {args.o}/LorPha_results/votu.meta.filter.txt -d {args.d}/vhost/virushostdb.tsv  ; "
